...
- 非 GPIO 引脚（电源、模拟等）归入 "SYS" 系列 unit
  - 若 SYS 引脚数 ≤ 40：单一 "SYS"
  - 若 SYS 引脚数 > 40：按 `subsystem` 分组后生成 "SYS1", "SYS2"...（同一子系统不跨 unit；单子系统可 > 40 且独占）
    - 子系统装箱只在**同一优先级层**内进行（unit ≤ 40）；不同优先级层之间**绝不混装**，即使当前 unit 仍有余量
    - 优先级层由 `SYS_SUBSYSTEM_PRIORITY_LEVELS` 配置；当前默认层级：`("power", "analog", "over") → ("crystal", "rf", "strapping") → ("audio", "mipi", "usb")`
    - 需要同优先级时，将多个 subsystem 放在同一层（同一个 tuple）即可
    - 若需要禁止同层贪心混装，在该层 tuple 中加入 `over`（该层每个 subsystem 都会强制另起一个 unit）
